...
def export_experiment_tasks(models, deploy_info):
	...
	for model_id, _ in models:
		...
		# deploy info
		experiment_tasks.append({ 'id': experiment_task_id, 'models': task_models, 'datasets': dataset_ids})
...
def get_args():
	...
	parser.add_argument('--serving-service-id', default='cd423197c94344c4b1d41a5cd2408770', help='serving service id')
	args = parser.parse_args()
	return args


if __name__ == "__main__":
	Task.init(project_name='Exporting2Standalone', task_name=str(datetime.now().date()), output_uri=True)
	args = get_args()
